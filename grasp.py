...
def LocalSearch(G, CL):
    ...
    Edges=getEdgesList(G)
    F=[]
    if len(CL[selectedCLIndex])>2 and len(set(selectedCL)-set(CL[selectedCLIndex]))==0:
        seenCouple=set()
        for node in CL[selectedCLIndex]:
            for voisin in CL[selectedCLIndex]:
                if node!=voisin and (voisin,node) not in seenCouple and ((node,voisin) not in Edges  and (voisin,voisin) not in Edges):
                    #on calcule cos+(i,C)+cos+(j,C) en sachant que i et j ne sont pas voisins dans G
                    costIJ=costP(node,G,CL[selectedCLIndex])-costP(voisin,G,CL[selectedCLIndex])
...
def grasp(G, Tmax):
    ...
    while current_time()-timestart < Tmax:
        ...
        if solutionCost(G,G2) < solutionCost(G,G1):
            G1=G2
...
if __name__ == '__main__':
    ...
    for i in range(1, n+1):
        if i not in G.keys():
            G[i] = list()
    print(getEdgesList(G))
 
    # done
    if(len(G) >0):
        print("--------")
        G1=grasp(G, 1000)
        print(getEdgesList(G1))
        print(" ")
        print(solutionCost(G,G1))
